...
:::info[Vous avez uniquement commandé un domaine ?]
Une prestation de domaine seul n'inclut pas de compte cPanel et ne donne pas accès à cet outil. Vous pouvez utiliser un
prestataire DNS externe, par exemple en suivant le guide pour [gérer la zone d'un domaine seul avec
Cloudflare](/nom-de-domaine/dns/utiliser-cloudflare-domaine-o2switch).
:::
