...
description: L'outil zone editor permet de visualiser et modifier l'ensemble de la zone DNS des noms de domaines configurés sur l'hébergement
...
Vous pouvez être amené à modifier une zone DNS pour :
  ...
  * prouver que vous êtes le propriétaire du domaine, par exemple, lors de l'[activation de Google Search Console](/guides/domaines/configurer-txt-google-search-console)
...
En cliquant sur le lien `Gérer` en face d'un des noms de domaines configurés sur l'hébergement dans la page d'accueil
...
Pendant très longtemps, il n'était pas possible de modifier les entrées de type MX depuis les outils d'éditions de zones
DNS de cPanel.
...
Voici quelques exemples de configurations qui sont couramment faites dans la zone DNS d'un nom de domaine.
  ...
  * [Comment ajouter l'enregistrement DNS TXT demandé lors de la validation de propriété de l'outil Google Console Search ?](/guides/domaines/configurer-txt-google-search-console)