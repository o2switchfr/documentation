...
d'autres [langages comme PHP, Python et NodeJS](/questions/langages-supportes-php-node-ruby-python).
...
versions des fichiers exécutables Ruby, notamment lorsque Ruby est utilisée comme une commande CLI, indépendante d'un projet web complet Ruby.

Pour un exemple d'utilisation, voir le [Comment modifier le $PATH ?](/questions/langages-supportes-php-node-ruby-python).
