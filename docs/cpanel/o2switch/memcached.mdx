...
performances des sites hébergés.
...
Nous recommandons l'utilisation de l'extension LiteSpeed Cache car l'extension est complète et permet également de gérer
le cache serveur LiteSpeed (qui est un autre type de cache, activable en complément).
