...
- vous avez beaucoup de site sur un hébergement et vous souhaitez **transférer** une partie des sites vers un autre
...
- vous êtes un Webmaster, vous hébergez les sites de vos clients sur un hébergement à votre nom. Un de vos clients
demande à être indépendant et a commandé un compte à son nom. Vous souhaitez **déplacer son site** vers son hébergement.

En résumé, il y a plusieurs raisons valables de vouloir **déplacer un site internet** d'un hébergement vers un autre 
hébergement. 

Il y a plusieurs méthodes pour **déplacer un site web**. Ces méthodes dépendent aussi du type de site à migrer. Dans ce
tutoriel, nous allons voir deux méthodes différentes : 
...
Avant de commencer, munissez-vous des accès cPanel pour vos deux hébergements. Vous pouvez retrouver cela dans 
les emails `Bienvenue chez o2switch` qui sont envoyés lors de l'ouverture des comptes d'hébergements. 
...
Si vous avez [créé un sous-compte d'hébergement (appelé une lune)](/cpanel/o2switch/univers-web-sous-comptes), alors
vous devriez déjà avoir le mot de passe puisque vous l'avez choisi. 
...
premier hébergement vers le second hébergement. Il y a plusieurs méthodes pour faire cela : 
- le plus simple, mais également le plus long, **en FTP**. En FTP, il faut d'abord récupérer les fichiers sur votre
ordinateur. Puis les retransférer de votre ordinateur vers le second hébergement. Il y a donc un double transfert de
données et votre ordinateur sert d'intermédiaire. Vous êtes potentiellement limité par la vitesse de votre connexion
internet. 
- de **serveur à serveur en SSH**. Cette méthode est plus **complexe** car demande de se connecter en SSH sur
l'hébergement et de lancer des lignes de commandes. Mais c'est également **bien plus rapide** car le transfert de
données se fait de serveur en serveur, sans avoir besoin d'un intermédiaire. De plus, cela est généralement plus fiable. 
...
Avant de commencer à copier les fichiers du site, il faut **identifier ou se trouvent les fichiers du site** sur votre 
hébergement. Pour faire cela, vous pouvez naviguer dans l'[outil Domaines
...
Si vous n'avez pas de résultat, alors suivez la même procédure mais avec [l'outil
sous-domaine](/cpanel/domaines/configuration-sous-domaine). Et si vous n'avez toujours pas de résultat, alors il est
probable que votre site se trouve dans `public_html`.

Ensuite, pour copier les fichiers du site du premier compte d'hébergement vers le second, vous avez le choix entre deux
méthodes : 

<Badge text="A" /> **En FTP**. C'est le plus simple, mais c'est également long et fastidieux. Cela nécessite de télécharger
les données sur votre ordinateur pour les renvoyer vers l'autre hébergement. Donc vous êtes limité par la vitesse de 
votre connexion internet. 

<Badge text="B" /> **En SSH**, de serveur à serveur. C'est la méthode la plus rapide, mais c'est un peu plus compliqué car
cela demande de se connecter en SSH sur l'hébergement et de lancer des commandes *(les commandes seront fournies)*.
...
Lancez Filezilla puis cliquez sur `Fichier` puis `Gestionnaire de sites`. Ajoutez un nouveau site qui correspond
à **l'hébergement source**. Puis recommandez la procédure mais avec **l'hébergement de destination** cette fois.

Aidez-vous des informations contenues dans le mail `Bienvenue chez o2switch`. Pour rappel, il faut utiliser les réglages
suivant : 
- `Protocole` : FTP
- `Hôte` : l'adresse de la forme `qqch.o2switch.net` qui est communiqué dans le mail ou retrouvable depuis votre espace
client dans le détails de la prestation d'hébergement 
- `Chiffrement` : connexion FTP simple. Si vous souhaitez utiliser une méthode différente, référez-vous au [tutoriel
sur Filezilla](/guides/configuration-logiciel/filezilla)
...
- transférer les données du site dans ce dossier avec un glisser-déposer sur Filezilla
...
Vous pouvez transférer les données de **serveur à serveur en SSH**. Le transfert de fichier avec cette méthode est
...
naviguer dans les dossiers de votre hébergement en ligne de commande, ou lister vos fichiers, vous pouvez
...
Pour transférer le contenu du site internet de l'hébergement source, vers l'hébergement de destination, il faut 
...
Quelques explications sur la commande précédente :
  ...
  * `-av` : des options qui permettent de conserver les permissions/dates des fichiers et d'activer le mode verbeux
  * `o2tuto@greek.o2switch.net` : l'identifiant de l'hébergement distant, suivi d'un @, suivi du nom du serveur distant
...
Après avoir lancé la commande, il faudra répondre à deux questions : 
  * Si c'est la première fois que vous vous connectez sur le serveur distant, l'outil vous demandera de valider la 
  signature du serveur, il suffit de répondre `yes` 
  * Ensuite il faudra noter le mot de passe de votre hébergement de destination pour vous authentifier 
...
Il faut laisser la commande s'exécuter jusqu'à qu'elle se termine. 
...
Il est probable que votre site internet utilise une base de données. Cette base de données doit être copiée, pour faire
cela, il faut : 
...
fichier](/cpanel/fichiers/gestionnaire-fichiers-web). Le fichier de configuration du site va contenir les informations
de connexion pour la base de données.

Le fichier à ouvrir dépend du CMS utilisé, dans les plus courant, vous avez : 
